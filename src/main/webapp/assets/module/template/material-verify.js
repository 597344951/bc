...
window.appInstince = new Vue({
  ...
  methods: {
    ...
    verifys(verify) {
      ...
      let url = `/resource/Material/verify/verifys/${verify}`;
      ajax_json_promise(url, 'post', this.multipleSelection).then(result => {
        if (result.status) {
          this.loadTpTypeData();
          this.loadTreeData();
        }
    ...
    verify(item, verify) {
      ...
      let url = `/resource/Material/verify/verify/${verify}`;
      ajax_json_promise(url, 'post', item).then(result => {
        if (result.status) {
          item.verify = verify;
