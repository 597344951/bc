  ...
  methods: {
      ...
      if ('SUCCESS' === response.state) {
        let photo = {
          ...
          thumbnail: response.thumbnail
            ? imageHost + response.thumbnail
            : imageHost + response.url,
          screenshot: response.screenshot
            ? imageHost + response.screenshot
            : '/images/video.png'
        }
    ...
    onEffectSelected() {
      if (this.effects.current) {
        ...
        this.wrap.demoUrl = `/pw/demo?url=${this.effects.current.url}&count=${
          this.effects.current.minimum
        }&effect=${
          this.effects.current.effect ? this.effects.current.effect : ''
        }`
    ...
    submitPreCheck() {
      ...
      if (
        this.wrap.photos.length > this.wrap.maximum ||
        this.wrap.photos.length < this.wrap.minimum
      ) {
    ...
    loadWraps(pageNum, pageSize) {
      get('/pw/list/' + pageNum + '/' + pageSize, resp => {
        if (resp.status) {
