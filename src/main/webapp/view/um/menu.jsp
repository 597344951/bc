            ...
            <table class="table table-hover treegrid-table table-bordered table-striped" id="menuTable" data-click-to-select="true" data-mobile-responsive="true">
                <thead class="treegrid-thead">
                    <tr>
                        ...
                        <th style="padding:10px;width:200px">菜单URL</th>
                        <th style="padding:10px;">授权标识</th>
                    </tr>
                </thead>
                <tbody>
                    <!--一级目录-->
                    <tr v-for="item in menuData" v-show="item.open || item.data.type == 0" v-on:click="menuItemClick(item)">
                        ...
                        <td style="width:180px" v-bind:class="'padding-'+item.data.type">
                            <span class="hand glyphicon" v-bind:class="item.children ? ( !item.openNext ? 'glyphicon-chevron-right':'glyphicon-chevron-down') :''"
                                v-on:click="menuItemOpenClose(item)">
                            </span>{{item.data.name}}
                        </td>
    ...
    var appInstince = new Vue({
        ...
        methods: {
                ...
                function next(obj, chi) {
                    if (chi) {
                        ...
                        for (let i = 0; i < chi.length; i++) {
                            ...
                            let o = v.data;
                            ay.push(o);
                            if (v.children && v.children.length > 0) next(o, v.children);
                        }
                        if (ay.length > 0) obj.children = ay;
                    }
                }
            ...
            saveOrUpdateMenu() {
                ...
                if (data.parentIds && data.parentIds.length >= 3) {
                    data.parentId = data.parentIds[data.parentIds.length - 2];
                } else if (data.parentIds && data.parentIds.length >= 2){
