                            ...
                            <el-menu class="el-menu-demo" mode="horizontal">
                                ...
                                <el-menu-item index="-3" class="normal-menu-item" disabled>
                                    <el-button-group class="control-button">
                                        <el-button size="small" :type="!dis_h_v?'primary':''" icon="el-icon-menu"
                                            @click="dis_h_v=false"></el-button>
                                        <el-button size="small" :type="dis_h_v?'primary':''" icon="el-icon-tickets"
                                            @click="dis_h_v=true"></el-button>
                                    </el-button-group>
                                </el-menu-item>
                                <el-menu-item index="-2" :style="{'float':'right'}" class="normal-menu-item" disabled>
                                    <el-pagination class="pagebar" :current-page="tpager.current" :page-sizes="[10, 20, 30]"
                                        :page-size="tpager.size" layout="total, sizes, prev, pager, next, jumper"
                                ...
                                <table class="dis-info-min">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <div>
                                                    <el-button class="no-margin-padding" type="text" size="small" v-if="pc.operate.process"
                                                        @click="getProcessState(pc)">进度</el-button>
                                                    <el-button type="text" size="small" v-if="pc.type == 7"
                                                        @click="viewTemplate(pc)">要求</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small" v-if="pc.operate.snapshot"
                                                        @click="view(pc)">预览</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small" v-if="pc.operate.verify_state"
                                                        @click="getVerifyState(pc)">审核状态</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="viewTerminal(pc)">发布终端</el-button>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="startMoreEdit(pc)" v-if="pc.operate.more_edit_start">开始编辑</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="moreEdit(pc)" v-if="pc.operate.more_edit">编辑</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="preCommitMoreEdit(pc)" v-if="pc.operate.more_edit_commit">编辑提交</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="preVerify(pc, 1)" v-if="pc.operate.verify">审核通过</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="preVerify(pc, 2)" v-if="pc.operate.verify">审核不通过</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="preVerify(pc, 3)" v-if="pc.operate.verify">审核放弃</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="openPublishPeriod(pc)" v-if="pc.operate.publish">发布</el-button>
                                                    <el-button class="no-margin-padding" type="text" size="small"
                                                        @click="discard(pc)" v-if="pc.operate.delete">移除</el-button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
                        ...
                        <el-table :data="processContent" border style="width: 100%">
                            ...
                            <el-table-column label="查看" width="200">
                                <template slot-scope="scope">
                                    ...
                                    <el-button type="text" size="small" v-if="scope.row.type == 7" @click="viewTemplate(scope.row)">要求</el-button>
        ...
        <el-dialog title="选择播放时间" :visible="publishPeriod.show" :show-close="false" width="500px">
            <el-form :model="publishPeriod.period" :rules="publishPeriod.rules" ref="publishPeriod" label-width="150px">
                <el-form-item label="开始日期" prop="startDate">
                    <el-date-picker v-model="publishPeriod.period.startDate" type="date" placeholder="开始日期"
                        value-format="yyyy-MM-dd 00:00:00" style="width: 300px"></el-date-picker>
                </el-form-item>
                ...
                <el-form-item label="工作/非工作日" prop="week">
                    <el-select class="period-input" v-model="publishPeriod.period.week" multiple collapse-tags
                ...
                <el-form-item label="每日时段" prop="time">
                    <el-time-picker style="width: 300px" is-range v-model="publishPeriod.period.time" range-separator="-"
                        value-format="H:mm" start-placeholder="开始时间" end-placeholder="结束时间" placeholder="选择时间范围">
                    </el-time-picker>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
