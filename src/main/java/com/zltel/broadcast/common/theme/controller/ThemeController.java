...
public class ThemeController extends BaseController {
    ...
    public ResponseEntity<String> changeTheme(@PathVariable("color") String cl) throws IOException {
        ...
        InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream(TEMPLATE_FILE_NAME);
        if (in == null)
            return ResponseEntity.ok().cacheControl(CacheControl.maxAge(30, TimeUnit.DAYS)).body("/** not found **/");
        ...
        if (user != null && !nc.equals(user.getTheme())) {
            user.setTheme(nc);
            // 调用保存
        ...
        // 设置允许浏览器缓存信息
        return ResponseEntity.ok().cacheControl(CacheControl.maxAge(30, TimeUnit.DAYS)).body(content);
    }
