...
public class SysRoleServiceImpl extends BaseDaoImpl<SysRole> implements SysRoleService {
    ...
    public R deleteSysRole(SysRole sysRole) throws Exception {
        if (sysRole == null || sysRole.getRoleId() == null) {
            throw RRException.makeThrow("请输入要删除的角色id"); // 删除用户一定需要uid，依据uid进行用户的删除
        }
        int count = 0;
        int countRoleMenu = 0;
        // 检测角色是否可以删除
        SysRole sr = this.selectByPrimaryKey(sysRole.getRoleId());
        if (sr == null) {
            throw RRException.makeThrow("删除角色不存在");
        }
        if (sr.getBuiltin()) {
            throw RRException.makeThrow("无法删除内置角色");
        }
        ...
        this.sysUserRoleMapper.deleteSysUserRole(sur);
        if (count == 1) { // 受影响的行数，判断是否全部删除
            ...
            throw RRException.makeThrow("删除失败");
        }
    }
